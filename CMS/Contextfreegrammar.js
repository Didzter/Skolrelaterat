...
// @author       You
// @match        http://webblabb.iit.his.se:82/*
// @grant        GM_getValue
// @grant        GM_setValue
// ==/UserScript==
...
//		var noun,verb,determiner,adjective,adverb,conjunction,modal;
	...
	return sentence.charAt(0).toUpperCase() + sentence.slice(1) + ". ";
        ...
        //script to create a product using the magento CMS.
    //note that "tab x" refers to the tabs with required input only! x represents the order, top to bottom.
    
    //Arrays containing the names to generate a random product name
    var nameList = new Array ("Orvars", "Svens", "Connys", "Konrads", "Stens", "Runes", "Glenns", "Robins", "Annas", "Birgittas", "Olles", "Louises", "Pelles", "He-mans", "Antons", "Evas", "Jeanettes", "Lillys", "Karins", "Gertruds", "Mulle-mecks", "Bosses", "Stinas", "Kalles", "Beatrices");
    var adjList = new Array ("Härliga", "Fina", "Fantastiska", "Snabba", "Färgglada", "Saftiga", "Goda", "Monströsa", "Kraftiga", "Sladdriga", "Suveräna", "Pompösa", "Abonnerade", "Adaptiva", "Accelererande", "Babblande", "Begagnade", "Kalibrerade", "Ambitiösa", "Strålande", "Skimmrande", "Flygande", "Flaxande", "Flytande", "Kusliga");
    var subjList = new Array ("Korvar", "Lådor och plank", "Grunkor", "Bilar", "Husdjur", "Tapeter", "Bakverk", "Konstverk", "Skärmar", "Skivor", "Klister", "Kläder", "Telefoner", "Cyklar", "Brädspel", "Vitvaror", "Påslakan och Tyger", "Båtar", "Godis", "Matvaror", "Byggklossar", "Humorkurser", "Trädgårdsartiklar", "Virtuella Upplevelser", "Hus och Slott");
    var priceList = new Array ("49", "99", "149", "249", "499", "649", "749", "899", "999", "1249", "1499", "1749", "1995", "2995", "3995", "4995", "9995", "14995", "19995", "24995", "49995", "99995", "149995", "249995", "499995", "999995");

    //variables tab 1
    var nameBox = document.getElementById("name");
    var descriptionBox = document.getElementById("description");
    var shortDescriptionBox = document.getElementById("short_description");
    var stockKeepingUnit = document.getElementById("sku");
    var weightBox = document.getElementById("weight"); //it's assumed that weight is in kg!
    var statusSelBox = document.getElementById("status");
    var visibilitySelBox = document.getElementById("visibility");
    
    //variables tab 2
  var priceBox = document.getElementById("price");
  var taxBox = document.getElementById("tax_class_id");
    
    //variables tab 3
    var quantityBox = document.getElementById("inventory_qty");
    
    
  /*
    example product
    nameBox.value = "Orvar's fantastiska korvar";
  descriptionBox.value = "Dom bästa korvarna på denna halva av jordklotet.";
    shortDescriptionBox.value = "Bra korvar";
    stockKeepingUnit.value = "9001";
    weightBox.value = "0.25";
    statusSelBox.value = 1;
    visibilitySelBox.value = 3;
    priceBox.value = "9";
    taxBox.value = 2;
    quantityBox.value = 500;
    */
  ...
  var randAdj = adjList[Math.floor(Math.random() * adjList.length)];
  var randSubj = subjList[Math.floor(Math.random() * subjList.length)];
  var randWeight = (Math.floor((Math.random() * 1000 + 1)));
  var randPrice = priceList[Math.floor(Math.random() * priceList.length)];
  var randQuantity = (Math.floor((Math.random() * 10000 + 1)));


    nameBox.value = randName +" "+ randAdj +" "+ randSubj;
    
  descriptionBox.value = "Dom bästa korvarna på denna halva av jordklotet.";
    shortDescriptionBox.value = "Bra korvar";
    //stockKeepingUnit.value = "9001";
    weightBox.value = randWeight;

    //set box to 1 or 2 (0 is "please select")
    statusSelBox.selectedIndex = (Math.floor((Math.random() * 2) + 1));
    //set box to 1, 2, 3 or 4 (0 is "please select")
    visibilitySelBox.selectedIndex = (Math.floor((Math.random() * 4) + 1));
    
    
    
    /*-----TAB 2-----*/
    
    priceBox.value = randPrice;
    
    //set box to 1, 2 or 3 (0 is "please select")
    taxBox.selectedIndex = (Math.floor((Math.random() * 3) + 1));
    
    
    
    /*-----TAB 3-----*/
    
    quantityBox.value = randQuantity;
    
    
    
   
     document.getElementById('description').value='';for(var i=0;i<20;i++){sentence=generate_sentence(0.5,0.5,0.5,0.5,null,null,null,null,null,null,null);document.getElementById('description').value+=sentence;
      }
       document.getElementById('short_description').value='';for(var i=0;i<5;i++){sentence=generate_sentence(0.5,0.5,0.5,0.5,null,null,null,null,null,null,null);document.getElementById('short_description').value+=sentence;
     ...
     var hej;
    hej = GM_getValue("count");    
    if (hej < 5000){
        hej++;
        GM_setValue("count", hej);
    }  
    else {
     hej = 0;
        GM_setValue("count", hej);
    ...
    document.getElementById("sku").value=GM_getValue("count");
